...
public class DAOCoordenada extends DAOFactory<Coordenada> {
	...
	/**
	 * Retorna a ultima coordenada enviada por uma viatura em um determinado
	 * setor
	 * 
	 * @param setor
	 *            int
	...
	public CoordenadaVO getUltimaCoordenadaViaturaSetor(int setor) {
		String strQuery = "SELECT c FROM Coordenada c WHERE c.id = "
				...
				+ "JOIN cid.viatura v " + "JOIN v.escalas e "
				+ "JOIN e.setor s " + "WHERE s.codigo = '" + setor + "')";
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<CoordenadaVO> getCoordenadasPorViatura(String viatura,
			Boolean foraDeArea) {
		List<CoordenadaVO> listaDeCoordenadas = new ArrayList<CoordenadaVO>();

		String strQuery = "SELECT c FROM Coordenada c "
				+ "WHERE c.viatura.codigo = '" + viatura
				+ "' and c.foraDeArea = '" + foraDeArea + "'";
