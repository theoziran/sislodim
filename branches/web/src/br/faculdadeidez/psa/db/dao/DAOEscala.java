...
public class DAOEscala extends DAOFactory<Escala> {
	...
	/**
	 * Verifica se as viaturas presentes na escala atual já estão cadastradas em
	 * outras escalas que estão entre o período inicial e final desta escala
	 * 
	 * @param vo
	 * @return
	 */
	...
	public String verificaViaturasNoutrasEscalasComMesmoDia(EscalaVO vo) {
		String strQuery = "SELECT v FROM Viatura v "
				+ "JOIN v.escalas sv "
				+ "WHERE :dataInicioEscala between sv.dataInicial and sv.dataFinal AND sv.ativo = 1 AND v.ativo = 1 and sv.codigo <> "
				+ vo.getCodigo();
		EntityManager em = getManager();
		Query query = em.createQuery(strQuery);
		...
		for (Viatura via : viaturas) {
			// verifica se alguma das viaturas retornadas estão dentre as
			// viaturas que
